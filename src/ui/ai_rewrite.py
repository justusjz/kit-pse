...
def select_dataset():
    path = filedialog.askopenfilename(title="Select Dataset", initialdir=BASE_DIR, filetypes=(("ARFF files", "*.arff"),))
    if path:
...
ttk.Checkbutton(load_dataset_frame, variable=print_data_graphs, text="Print data graphs?", bootstyle=INFO).grid(row=0, column=1)
...
def train_model():
    ...
    ml_trainer.train(model_name.get(), auto_feature.get(), selected_features, dataset_path.get())
...
ttk.Checkbutton(select_features_screen, variable=auto_feature, text="auto feature selection", bootstyle=INFO, command=toggle_auto_feature_selection).pack()
...
ttk.OptionMenu(train_model_frame, model_name, *ml_trainer.get_supported_model(), bootstyle=INFO).pack()
ttk.Button(train_model_frame, text="Train Model", command=train_model, bootstyle=SUCCESS).pack()
...
def update_features():
    ...
    for index, field in enumerate(dataset_metadata.names()):
        ...
        checkbutton = ttk.Checkbutton(features_frame, text=field, variable=var, command=on_check, bootstyle=INFO)
...
def open_model_charts_window(predictions_mapped, Y_test_mapped):
    ...
    ttk.Label(tab, text=f"Accuracy: {Accuracy:.7f}").pack()
    ttk.Label(tab, text=f"Sensitivity: {Sensitivity:.7f}").pack()
    ttk.Label(tab, text=f"Precision: {Precision:.7f}").pack()
    ttk.Label(tab, text=f"F1 Score: {F1_score:.7f}").pack()
    ttk.Label(tab, text=f"Recall: {Recall:.7f}").pack()
...
def create_save_close_bar(diagram_frame, fig=None, title="fig"):
    ...
    save_button = ttk.Button(
        button_frame, text="💾", bootstyle=INFO, command=lambda: save_fig(fig, title)
    )
...
ttk.Button(
    analysis_screen, text="Go Back", command=lambda: select_features_screen.tkraise(), bootstyle=SECONDARY
).pack()
