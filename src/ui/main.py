...
dataset_path = tk.StringVar(value='C:/Users/Kosta/IdeaProjects/kddtest4/archive/KDDTrain+.arff')
...
def open_model_charts_window():
  ...
  predictions_mapped = ["normal", "normal", "normal", "attack", "attack", "attack", "attack", "attack"]
  Y_test_mapped = ["attack", "normal", "normal", "normal", "attack", "normal", "normal", "attack"]

  #scores
  Accuracy = metrics.accuracy_score(Y_test_mapped, predictions_mapped)
  Sensitivity = metrics.recall_score(Y_test_mapped, predictions_mapped, pos_label="attack")
  Precision = metrics.precision_score(Y_test_mapped, predictions_mapped, pos_label="attack")
...
def open_data_charts_window():
  ...
  ndf = data_cleaned[[col for i, col in enumerate(dataset_metadata.names()) if dataset_metadata.types()[i] == 'numeric' and data_cleaned[col].nunique() > 1]]
  corr = ndf.corr()
  ...
  fig, ax = plt.subplots(figsize=(6, 4))
  ax.scatter(X_pca[:, 0], X_pca[:, 1], alpha=0.5, marker="o", c=["red" if x == "attack" else "blue" for x in data_cleaned["class"]])
...
def create_save_close_bar(diagram_frame, fig=None, title="fig"):
  ...
  close_button = tk.Button(button_frame, text="X", fg="red", command=diagram_frame.destroy, bd=0)
  close_button.grid(row=0, column=1, padx=1, sticky="e")
  if fig is None:
    return
  save_button = tk.Button(button_frame, text="💾", fg="blue", command=lambda:save_fig(fig, title), bd=0)
